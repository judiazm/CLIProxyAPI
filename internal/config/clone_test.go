...
func TestCloneForRuntimeDeepCopiesConfig(t *testing.T) {
	...
	if clone.APIKeys[0].APIKey != "client-key" {
		t.Fatalf("clone.APIKeys[0] = %q, want client-key", clone.APIKeys[0].APIKey)
	}
	...
	if cfg.APIKeys[0].APIKey != "mutated-client-key" {
		t.Fatalf("cfg.APIKeys[0] = %q, want mutated-client-key", cfg.APIKeys[0].APIKey)
	}
...
func sampleCloneRuntimeConfig() *Config {
	...
	return &Config{
		SDKConfig: SDKConfig{
			APIKeys: NewAPIKeyEntries("client-key"),
